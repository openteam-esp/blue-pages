...
task :generate_pdfs => ['generate_pdf:government', 'generate_pdf:zdrav_department', 'generate_pdf:family_department']
...
namespace :generate_pdf do
  ...
  desc 'Генерация справочника Департамента здравоохранения'
  task :zdrav_department => :environment do
    BluePagesBook.new(:root => Category.find_by_slug(:zdrav_department)).generate_pdf
  end
