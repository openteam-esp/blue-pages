class Innorganization < Category
  ...
  private
    def send_messages_on_create
      MessageMaker.make_message('esp.blue-pages.cms', :index_organization, id) if Rails.env.production?
    end

    alias :send_messages_on_update :send_messages_on_create

    def send_messages_on_destroy
      MessageMaker.make_message('esp.blue-pages.cms', :remove_organization, id) if Rails.env.production?
    end
end
